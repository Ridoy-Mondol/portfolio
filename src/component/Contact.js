...
const Contact = () => {
    ...
    const getData = async (elem) => {
        ...
        if (data.name.length > 0 && data.phone.length > 0 && data.email.length > 0 && data.subject.length > 0 && data.message.length > 0) {
            toast.success(`Thank you. Your fullname is ${data.name}, Your phone no. is ${data.phone}, Your email is ${data.email}, Your subject is ${data.subject}, Your message is ${data.message}`);
    ...
    const isLinkedInIntersecting = linkedInEntry ? linkedInEntry.isIntersecting : false;
    const isFacebookIntersecting = facebookEntry ? facebookEntry.isIntersecting : false;
    const isWhatsappIntersecting = whatsappEntry ? whatsappEntry.isIntersecting : false;
    ...
    const isLocationIntersecting = locationEntry ? locationEntry.isIntersecting : false;

    const isNameIntersecting = nameEntry ? nameEntry.isIntersecting : false;
    const isEmailInputIntersecting = emailInputEntry ? emailInputEntry.isIntersecting : false;
    const isPhoneIntersecting = phoneEntry ? phoneEntry.isIntersecting : false;
    const isMessageIntersecting = messageEntry ? messageEntry.isIntersecting : false;
                            ...
                            <div ref={phoneRef} id="contact-phone">
                                <div className="input-flex mt-3">
                                    <input 
                                        ...
                                        required= "required" 
                                        onChange={userData} 
                                        className={`input-inline ${isPhoneIntersecting ? 'rightIn' : ''} ${mode ? "input-light" : ""}`} 
                                    />
                                    <input 
                                        type="text" 
                                        placeholder="Subject" 
                            ...
                            <div ref={messageRef} id="contact-message">
                                <div className={`${isMessageIntersecting ? 'rightIn' : ''}`}>
                                    <textarea 
                                        ...
                                        required= "required" 
                                        onChange={userData} 
                                        className={`input-div input-msg mt-3 ${mode ? "input-light" : ""}`} 
                                    />
                                </div>
                            </div>
